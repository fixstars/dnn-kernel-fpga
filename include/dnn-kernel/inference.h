...
#pragma HLS inline
  ...
  static const int kChannels[] = {3, 20, 50, 3200, 500, 10};

#ifndef __SYNTHESIS__
  float* x1 = new float[kWidths[0] * kHeights[0] * kChannels[1]];
  float* x2 = new float[kWidths[0] * kHeights[0] * kChannels[1]];
  float* x3 = new float[kWidths[1] * kHeights[1] * kChannels[1]];
  float* x4 = new float[kWidths[1] * kHeights[1] * kChannels[2]];
  float* x5 = new float[kWidths[1] * kHeights[1] * kChannels[2]];
  float* x6 = new float[kWidths[2] * kHeights[2] * kChannels[2]];
  float* x7 = new float[kChannels[4]];
  float* x8 = new float[kChannels[4]];
#else
  ...
  float x7[kChannels[4]];
  float x8[kChannels[4]];
#endif
  ...
  // 3rd layer
  linear(x6, weight2, bias2, kChannels[3], kChannels[4], x7);
  relu(x7, kChannels[4], x8);

  // 4th layer
  linear(x8, weight3, bias3, kChannels[4], kChannels[5], y);
